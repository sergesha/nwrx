...
export const INTERCOM_SERVICE_DEFAULT_OPTIONS_TOKEN: InjectionToken<IntercomOptions> = new InjectionToken(
...
export class IntercomService implements OnDestroy {
    ...
    constructor(@Inject(INTERCOM_SERVICE_DEFAULT_OPTIONS_TOKEN) defaultOptions: IntercomOptions) {
    ...
    public read<T>(keys?: string | Array<string>): Observable<IntercomData<T>> {
        ...
        if (!this.subscriptions.has(subKey)) {
            this.subscriptions.set(
                subKey,
                this.onUpdate$.asObservable().pipe(
                    filter((change: IntercomData<T>) => {
                        return (
                            change &&
                            (typeof keys === 'undefined' ||
                                Object.keys(change).every((key: string) => keys.includes(key)))
                        );
                    }),
                    shareReplay({
                        bufferSize: 1,
        ...
        return this.subscriptions.get(subKey).pipe(startWith(initData));
    }
